...
- Every refund is recorded in the store ledger AFTER the commit (idempotent via refundStatus guard): fulfilled orders use `recordRefund` (debits settlement); never-fulfilled orders use `recordCustomerRefund` (REFUND_EXPENSE/ACCOUNTS_RECEIVABLE — must NOT touch settlement, it was never credited).
